...
JNI_FUNC(initX264)(JNIEnv *env, jclass clazz, jint width,
    ...
    swsVideo = new SwsVideo(width, height, x264Encoder);
}
...
JNI_FUNC(pushRGB)(JNIEnv *env, jclass clazz, jbyteArray rgb_bytes) {
    jbyte *rgb = env->GetByteArrayElements(rgb_bytes, 0);
    swsVideo->encodeRGB((uint8_t*)rgb);
    env->ReleaseByteArrayElements(rgb_bytes, rgb, 0);
}
